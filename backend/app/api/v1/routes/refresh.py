...
from ..dependencies import get_redis, get_current_user_jwt, get_user_spotify_service, get_user_google_service
...
async def refresh_user_session(
    ...
    try:
        spotify_refresh_token = redis.get(f'{old_jwt}_spotify_refresh_token')
        if spotify_refresh_token is not None:
            new_access_token = spotify_service.refresh_access_token(
                spotify_refresh_token.decode('utf-8'))
        ...
        google_refresh_token = redis.get(f'{old_jwt}_google_refresh_token')
        if google_refresh_token is not None:
            new_access_token = google_service.refresh_access_token(
                google_refresh_token.decode('utf-8')
            )
            if new_access_token is not None:
                redis.set(
